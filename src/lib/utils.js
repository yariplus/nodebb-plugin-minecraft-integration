...
export const parseFormatCodes = text => {
  if (!text) return ''
...
export const formatAddress = server => {
...
export const isIP = string => !isNaN(parseInt(string.substring(0, 1)))

export const verifyAddress = (server, next) => {
  server.host = '0.0.0.0'
  server.port = '25565'
  next(null, server)
}

export const getPingStampsByRecency = (minutes, trim, next) => {
  const stamps = []
...
export const getPingStampsByRange = (start, stop, trim, next) => {
  next()
}
...
export const trimUUID = uuid => uuid.replace(/-/g, '')

export const untrimUUID = uuid => {
