...
function getServer(data, next)
{
	...
	Backend.getServerConfig({sid: data.sid}, function (err, config)
	{
		if (err) return next(err);

		var socket = null;
		if (config.socketid && NodeBB.SocketIO.server.sockets.connected[config.socketid])
		{
			socket = NodeBB.SocketIO.server.sockets.connected[config.socketid];
		}

		return next(null, {server: config, socket: socket});
	});
}
...
Controller.PlayerVotes = function (data, callback)
{
	...
	Backend.getUser({name: name}, function (err, user)
	{
