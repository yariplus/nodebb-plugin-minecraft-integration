...
Controller.sendStatusToUsers = function (status, callback) {
	try {
		if (status.players && typeof status.players === 'string') status.players = JSON.parse(status.players);
		if (status.modList && typeof status.modList === 'string') status.modList = JSON.parse(status.modList);
		if (status.pluginList && typeof status.pluginList === 'string') status.pluginList = JSON.parse(status.pluginList);
	}catch(e){
