(function(Utils){
	...
	var styleResets = "display:inline-block; font-style: normal; text-decoration: none; font-weight: normal;";
	...
	Utils.parseVersion = function (version) {
		if (version) {
			var parsed = /\(MC:[^\d]*(\d+\.\d+(\.\d+)?)[^\d]*\)/.exec(version);
			if (parsed && parsed[1]) {
				version = parsed[1];
			}
		}else{
	...
	Utils.getUUID = function (name, next) {
	...
	Utils.getName = function (id, next) {
		console.log("Fetching Name for " + id);
		request({url: 'https://sessionserver.mojang.com/session/minecraft/profile/' + id, json: true}, function (err, response, body) {
	...
	Utils.parseMinutesDuration = function (minutes, locale) {
		return humanize(minutes * 60 * 1000, { language: locale || 'en', units: ['d', 'h', 'm']});
	};

	Utils.getHumanTime = function (stamp) {
		return humanTime(stamp);
	};
