...
Config.getConfig = function (data, next) {
	data.MinecraftIntegration = {
		avatarSize   : parseInt(Config.settings.get('avatarSize'), 10),
		avatarStyle  : Config.settings.get('avatarStyle'),
		debug        : !!parseInt(Config.settings.get('debug'), 10),
		pingExpiry   : parseInt(Config.settings.get('pingExpiry'), 10),
		showPrefixes : !!parseInt(Config.settings.get('showPrefixes'), 10)
	};
	next(null, data);
};
...
Config.getPlayerExpiry = function () {
	// A day
	// TODO: Make it a config option.
