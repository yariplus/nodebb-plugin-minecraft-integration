...
Config.settings = new NodeBB.Settings('minecraft-integration', '0.6.0', defaultSettings);
...
Config.getConfigValue = function (data, next) {
	if (!data || !data.key) {
		next("[[error:no-key]]");
	}else if (data.key === 'APIKey' || data.key === 'servers') {
...
function getServer(sid, callback) {
...
Config.getServerConfig = function (data, next) {
	...
	if (!(data && data.sid !== null && typeof data.sid !== 'undefined' && data.sid > -1)) return next(null, null);
...
Config.getServerByName = function (name, callback) {
	async.detect(Config.settings.get('servers'), function(server, next){
		next((server && server.name && server.name === name) ? true : false);
	}, function(server){
...
Config.setServerSocket = function (data) {
	...
	if (!(data && data.sid && data.socketid)) return;

	NodeBB.db.setObjectField('mi:server:' + data.sid + ':config', 'socketid', data.socketid);
