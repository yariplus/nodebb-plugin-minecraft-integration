...
Config.getAvatar = function (data, callback) {
		...
		if (!avatar) {
			...
			request({url: url, encoding: null}, function (err, response, body) {
				...
				NodeBB.db.set('mi:' + player + ':avatar', base64, function (err) {
					...
					// Expire the avatar.
					NodeBB.db.expire('mi:' + player + ':avatar', Config.getAvatarExpiry());
			...
			if (data.base64) {
				...
				callback(null, new Buffer(avatar, 'base64'));
			}
		}
	});
};
