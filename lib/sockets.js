	...
	if (Sockets.sockets[sid] === void 0) {
		...
		if (Sockets.sockets[sid]._callbacks.connect.length === 1) {
			...
			Sockets.sockets[sid].on('PlayerJoin', function (data) {
				data = data.split(":");
				var player = {id: data[0] || 'unknown uuid', name: data[1] || 'unknown name'};

				console.log('[Minecraft Integration] [' + server.name + '] Player ' + player.name + ' ' + player.id + ' has logged on.');
				Controller.sendPlayerJoinToUsers(player);
			});
			...
			Sockets.sockets[sid].on('PlayerQuit', function (data) {
				...
				console.log('[Minecraft Integration] [' + server.name + '] Player ' + player.name + ' ' + player.id + ' has quit.');
				Controller.sendPlayerQuitToUsers(player);
			});
		}
	}else{
