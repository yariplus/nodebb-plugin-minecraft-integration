...
function initServer(sid) {
	...
	if (Sockets.sockets[sid] === void 0) {
		Sockets.sockets[sid] = SocketIO(uri, { reconnection: false, transports: ['websocket'] });
		...
		if (Sockets.sockets[sid]._callbacks.connect.length === 1) {
			...
			Sockets.sockets[sid].on('someevent', function (data) {
				console.log('[Minecraft Integration] [' + server.name + '] Received event TEST from Minecraft API at ' + uri + ' with payload: ' + data);
			});
