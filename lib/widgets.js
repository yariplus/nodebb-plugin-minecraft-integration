...
function formatWidget(widget, callback) {
	...
	async.parallel({
		...
		server: async.apply(Backend.getServerConfig, {sid: widget.data.sid})
	}, function (err, payload) {

		if (err) return callback(err);
		if (!payload.server) return callback(new Error("No Config for widget render sid: " + widget.data.sid));
		...
		widget.data.config  = payload.server.config;
		widget.data.address	= payload.server.config.address;

		widget.data.name	= Utils.parseMCFormatCodes(payload.server.config.name);
