...
Backend.getPlayersFromUid = function (uid, next) {
	NodeBB.db.getSortedSetRangeByScore('yuuid:uid', 0, 1, uid, uid, function (err, yuuids) {
		if (err || !yuuids || !yuuids.length) return next();
...
Backend.deleteProfile = Backend.deletePlayer = function (data, next) {
	Backend.getPlayer(data, function (err, profile) {
		if (err) return next(err);
	...
	NodeBB.db.getSortedSetRange('users:joindate', 0, -1, function (err, uids) {
		async.each(uids, function (uid, next) {
			NodeBB.db.getObjectFields('user:' + uid, fields, function (err, userData) {
				if (!!userData.yuuid) {
					Backend.getPlayerFromUuid(userData.yuuid, function (err, profile) {
