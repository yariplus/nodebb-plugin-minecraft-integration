...
Backend.getPlayer = function (options, next) {
	if (!options || !(options.name || options.id)) {
		console.log("getPlayer() invalid options:", options);
		return next(true, {});
	}else if (options.name) {
		Backend.getProfile(options, function (err, player) {
			if (!player || !player.id) return next();

			NodeBB.db.getObject('yuuid:' + player.id, function (err, player) {
				player.id = id;
				next(err, player);
			});
		});
		...
		NodeBB.db.getObject('yuuid:' + id, function (err, player) {
			player.id = id;
			next(err, player);
		});
	}
...
Backend.getProfile = function (data, next) {
	if (!data || !(data.name || data.id)) {
		console.log("getProfile() invalid data.");
		return next();
	// TODO: Always call the uuid 'id', except in user objects, where it's yuuid.
	}else if (data.id) {
		var id = data.id;

		NodeBB.db.exists('yuuid:' + id, function (err, exists) {
			if (err) return next(err);

			if (exists) {
				NodeBB.db.getObject('yuuid:' + id, function (err, player) {
					if (err || !player) console.log("Backend.getProfile() Couldn't get player object for id: " + id);

					next(err, {id: id, name: player.name});
				});
			}else{
				Utils.getPlayerNameUsingUUID(id, function (err, name) {
					if (err || !name) {
						console.log("Backend.getProfile() Couldn't get name from Mojang for id: " + id);
					}else{
						NodeBB.db.setObject('yuuid:' + id, {name: name, lastonline: Math.round(Date.now()/60000) * 60000, playtime: 0});
					}

					next(err, {id: id, name: name});
				});
			}
		});
	}else if (data.name) {
		...
		NodeBB.db.exists('mi:name:' + name, function (err, exists) {
			if (err) return next(err);

			if (exists) {
				NodeBB.db.get('mi:name:' + name, function (err, id) {
					return next(err, {id: id, name: name});
				});
			}else{
				Utils.getPlayerUUID(name, function (err, id) {
					if (err) {
						console.log("Backend.getProfile() Couldn't get UUID from Mojang for user: " + name);
					}else{
...
Backend.getServerStatus = function (data, callback) {
	...
	NodeBB.db.getObject('mi:server:' + sid, function (err, status) {
		if (err) {
			...
			callback("Error: No status for server " + data.sid + ": " + Config.getServer(data.sid).name, { });
		}else{
			try {
	...
	Backend.getAvatarList({}, function (err, avatarList) {
		...
		async.each(avatarList, function (name, next) {
			Backend.getAvatarByPlayer({name: name, base64: true}, function (err, base64) {
				...
				Backend.getProfile({name: name}, function (err, profile) {
...
Backend.getTopPlayersByPlaytimes = function (data, callback) {
	...
	NodeBB.db.getSortedSetRevRangeByScoreWithScores('yuuid:playtime', 0, data.show, '+inf', 0, function (err, data) {
		async.map(data, function (value, next) {
			Backend.getProfile({id: value.value}, function (err, profile) {
				if (err) return next(err);
