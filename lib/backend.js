			...
			NodeBB.db.getSortedSetRange('yuuid:' + yuuid + ':uids', 0, -1, function (err, uids) {
				...
				async.each(uids, function (uid, next) {
					...
					NodeBB.db.getObjectFields('user:' + uid, fields, function (err, userData) {
						var uuid = userData.yuuid;

						Backend.getPlayersFromUuids([uuid], function (err, players) {
							if (players && players[0]) users.push(players[0]);
							next();
						});
					});
				}, next);
