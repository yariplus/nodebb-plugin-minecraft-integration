...
Backend.updateServers = function () {
	...
	async.each(servers, function (server, next) {
		if (server.active){
			...
			async.waterfall([
				function (next) {
					Backend.doServerStatusPing(server, next);
				},
				function (ping, next) {
					NodeBB.db.setObject('mi:server:' + servers.indexOf(server), ping, function (err) {
						NodeBB.db.expire('mi:server:' + servers.indexOf(server), Config.getPingExpiry(), function (err) {
							next(err, ping);
						});
					});
				},
				function (ping, next) {
					NodeBB.db.listPrepend('mi:server:' + servers.indexOf(server) + ':pings', updateTime, function (err) {
						NodeBB.db.expire('mi:server:' + servers.indexOf(server) + ':pings', Config.getPingExpiry(), function (err) {
							next(err, ping);
						});
					});
				},
				function (ping, next) {
					NodeBB.db.setObject('mi:server:' + servers.indexOf(server) + ':ping:' + updateTime, ping, function (err) {
						NodeBB.db.expire('mi:server:' + servers.indexOf(server) + ':ping:' + updateTime, Config.getPingExpiry(), function (err) {
							next(err, ping);
						});
					});
				}
...
Backend.doServerStatusPing = function (server, callback) {
	mcping('0.0.0.0', '25565', function(err, resp) {
...
Backend.doServerStatusPingOld = function (data) {
