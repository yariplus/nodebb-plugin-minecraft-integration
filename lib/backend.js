...
Backend.getServerStatus = function (data, next) {
	...
	async.parallel({
		...
		if (parseInt(server.config.hidePlugins, 10)) status.pluginList = [ ];
...
Backend.getServerPlugins = function (data, next) {
	...
	async.parallel({
		...
		if (!results.pluginList || parseInt(results.server.config.hidePlugins, 10)) {
			next(null, [ ]);
		}else{
			try {
...
Backend.updateServerStatus = function (status, next) {
	if (typeof status.players !== 'string') status.pluginList = JSON.stringify(status.pluginList);
	if (typeof status.pluginList !== 'string') status.pluginList = JSON.stringify(status.pluginList);
	if (typeof status.modList !== 'string') status.pluginList = JSON.stringify(status.pluginList);
	async.waterfall([
