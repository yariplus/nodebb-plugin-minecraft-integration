...
Backend.getServerStatus = function (data, next) {
	...
	async.parallel({
		...
		server: async.apply(Backend.getServerConfig, {sid: sid})
		...
		if (err) return next(err);
		if (!server) return next(new Error("Backend.getServerStatus() No server exists for SID " + sid));
		if (!status) return next(new Error("Backend.getServerStatus() No status exists for SID " + sid + " named " + server.config.name));
		...
		if (server.config.hidePlugins) status.pluginList = [ ];
...
Backend.getServersConfig = function (data, callback) {
	...
	NodeBB.db.getSortedSetRange('mi:servers', 0, -1, function (err, sids) {
		...
		if (!sids) return callback(null, payload);

		if (data && data.sort) {
			if (data.sort === "sid") sids = sids.sort();
		}else{
			sids = sids.sort();
		}
		...
		async.each(sids, function (sid, next) {
			...
			NodeBB.db.getObject('mi:server:' + sid + ':config', function (err, config) {
				...
				payload.push({config: config, sid: sid});
				next();
...
Backend.getServerConfig = function (data, callback) {

	if (!(data && data.sid)) return callback(new Error("Backend.getServerConfig() No SID.", data));

	NodeBB.db.getObject('mi:server:' + data.sid + ':config', function (err, config) {

		if (err) return callback(err);
		if (!config) return callback(new Error("Backend.getServerConfig() invalid SID: " + data.sid));

		callback(null, {sid: data.sid, config: config});

	});

};

Backend.setServerConfig = function (data, callback) {

	if (!(data && data.sid)) return callback(new Error("Backend.setServerConfig() No SID."));

	data.config         = data.config || {};
	data.config.name    = data.config.name || "Unnamed Server";
	data.config.address = data.config.address || "example.com";

	// Score is used for sorting.
	NodeBB.db.sortedSetAdd('mi:servers', Date.now(), data.sid);
	NodeBB.db.setObject('mi:server:' + data.sid + ':config', data.config, callback);

};

Backend.getSidUsingAPIKey = function (key, callback) {
	...
	Backend.getServersConfig({}, function (err, servers) {
		...
		servers.forEach(function (server) {
			...
			if (server.config.APIKey === key) return callback(null, server.sid);

		});

		return callback(new Error("Config.getSidUsingAPIKey() No server found."));
...
Backend.updateServerStatus = function (status, next) {
	...
	async.waterfall([
		...
		async.apply(NodeBB.db.setObject,      'mi:server:' + sid, status),
		async.apply(NodeBB.db.expire,         'mi:server:' + sid, Config.getPingExpiry()),
		async.apply(NodeBB.db.setObjectField, 'mi:server:' + sid + ':ping:' + updateTime, 'players', status.players),
		async.apply(NodeBB.db.setObjectField, 'mi:server:' + sid + ':ping:' + updateTime, 'tps', tps),
		async.apply(NodeBB.db.expire,         'mi:server:' + sid + ':ping:' + updateTime, Config.getPingExpiry()),
		async.apply(NodeBB.db.listPrepend,    'mi:server:' + sid + ':pings', updateTime)
	], next);
