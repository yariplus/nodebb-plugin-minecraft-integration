...
Backend.getPlayers = function (options, next) {
	...
	NodeBB.db.getSortedSetRevRange(set, min, max, function (err, uuids) {
		async.each(uuids, function (id, next) {
			Backend.getPlayerFromUuid(id, function (err, profile) {
...
Backend.getPlayer = function (data, next) {
	if (!data || !(data.id || data.name || data.uid)) return next(new Error("No data for player lookup."));
	if (data.id) return Backend.getPlayerFromUuid(data.id, next);
	if (data.name) return Backend.getPlayerFromName(data.name, next);
	if (data.uid) return Backend.getPlayersFromUid(data.uid, next);
};

Backend.getPlayerFromName = function (name, next) {
	...
	NodeBB.db.get('mi:name:' + name, function (err, id) {
		...
		if (id) return Backend.getPlayerFromUuid(id, next);

		// If not in cache, query Mojang.
		Utils.getUUID(name, function (err, id) {
			...
			// Return the Profile.
			return Backend.getPlayerFromUuid(id, next);
		});
	});
};

Backend.getPlayerFromUuid = function (id, next) {
	...
	NodeBB.db.getObject('yuuid:' + id, function (err, profile) {
		...
		async.waterfall([
			function (next) {
				if (profile.uid) {
					NodeBB.User.getUserData(profile.uid, function (err, user) {
						profile.user = user;
						next();
					});
				}else{
			...
			function (next) {
				...
				if (!profile.name || (!profile.lastupdate || (Date.now() - parseInt(profile.lastupdate, 10) > Config.getPlayerExpiry()))) {
		...
		// console.log("expiry:" + Config.getPlayerExpiry());
		// console.log("needs update:" + !!(!profile.name || (!profile.lastupdate || (Date.now() - parseInt(profile.lastupdate, 10) > Config.getPlayerExpiry()))));
	});
};

// Get the primary player linked to a user.
Backend.getPlayerFromUid = function (uid, next) {
	NodeBB.User.getUserField(uid, 'yuuid', function (err, yuuid) {
		Backend.getPlayerFromUuid(yuuid, next);
	});
};

// Get the all players linked to a user.
Backend.getPlayersFromUid = function (uid, next) {
	async.waterfall([
		async.apply(NodeBB.db.getSortedSetRange, 'uid:' + uid + ':yuuids', 0, -1),
		function (yuuids) {
			async.map(yuuids, Backend.getPlayerFromUuid, next);
		}
	], next);
};
	...
	Backend.getAvatarList({}, function (err, avatarList) {
		...
		async.each(avatarList, function (name, next) {
			getAvatar(name, function (err, base64) {
				...
				Backend.getPlayerFromName(name, function (err, profile) {
...
function fetchAvatar(name, next) {
	...
	async.parallel({
		...
		profile: async.apply(Backend.getPlayerFromName, name)             // We need this for cdns that use uuids.
...
//////////////////////
// Registered Users //
// yuuid:uid        // Depreciated SortedSet
// playerkey:uid    // SortedSet
// yuuids:linked    // SortedSet
//////////////////////

Backend.resetPlayerKey = function (data, next) {
	next = next || function(){};

	if (!(data && data.uid)) return next(new Error("Bad data sent to Backend.resetPlayerKey."));

	var	uid = data.uid,
		key = Utils.getKey();

	Backend.getPlayerKey(data, function (err, result) {
		if (err) return next(err);

		NodeBB.db.sortedSetRemove('playerkey:uid', result.key, function(){
			Backend.getPlayerKey(data, next);
		});
	});
};

// Gets and/or creates the player key.
Backend.getPlayerKey = function (data, next) {
	next = next || function(){};

	if (!(data && data.uid)) return next(new Error("Bad data sent to Backend.getPlayerKey."));
	...
	NodeBB.db.getSortedSetRangeByScore('playerkey:uid', 0, 1, uid, uid, function (err, key) {
		...
		return next(err, {key: key});
	});
};

// Get all linked users.
Backend.getUsers = Backend.getRegisteredUsers = function (options, next) {
	...
	NodeBB.db.getSortedSetRange('yuuids:linked', 0, -1, function (err, yuuids) {
			...
			NodeBB.db.getSortedSetRange('yuuid:' + yuuid + ':uids', 0, -1, function (err, uids) {
				...
				async.each(uids, function (uid, next) {
					...
					NodeBB.db.getObjectFields('user:' + uid, fields, function (err, userData) {
						...
						Backend.getPlayer({id: yuuid}, function (err, player) {
...
Backend.getUser = Backend.getRegisteredUser = function (options, next) {
...
function getUserFromUuid(id, extraFields, next) {
	...
	NodeBB.db.getObjectField('yuuid:' + id, 'uid', function (err, uid) {
		if (err) return next(err);
		NodeBB.User.getUserFields(uid, fields, function (err, userData) {
			if (err) return next(err);
...
Backend.deleteUser = function (data, next) {
	...
	// Remove all linked players.
	Backend.getPlayersFromUid(uid, function (err, players) {
		async.each(players, Backend.deletePlayer, function (err) {
			// Remove primary yuuid from user object.
