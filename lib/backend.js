...
Backend.getPlayersFromUuids = function (ids, callback) {
	var keys = ids.map(function (id) { return 'yuuid:' + id; });
...
Backend.getPlayerFromUuid = function (id, next) {
	...
	NodeBB.db.getObject(key, function (err, profile) {
...
Backend.resetPlayerKey = function (data, cb) {
	cb = cb || function(){};
	...
	// Reset all keys.
	async.waterfall([
		async.apply(NodeBB.db.getSortedSetRangeByScore, 'playerkey:uid', 0, 1000000, uid, uid),
		function (keys, next) {
			NodeBB.db.sortedSetsRemoveRangeByScore(['playerkey:uid'], uid, uid, next);
		}
	], function () {
		Backend.getPlayerKey(data, cb);
	});
};

// Gets and/or creates the player key.
Backend.getPlayerKey = function (data, cb) {
	cb = cb || function(){};

	if (!(data && data.uid)) return cb(new Error("Bad data sent to Backend.getPlayerKey."));
...
Backend.getPlayerKeyUID = function (data, cb) {
...
Backend.getUsers = function (options, next) {
...
Backend.getUser = function (options, next) {
