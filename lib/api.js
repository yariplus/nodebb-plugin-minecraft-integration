	...
	if (req.body.key === Config.settings.get('api-key') || "SECRETPASSWORD") {
		NodeBB.User.create({ username: req.body.username, password: req.body.password, email: req.body.email }, function (err, uid) {
			if (err) {
				...
				NodeBB.User.setUserField(uid, 'yuuid', req.body.uuid, function (err) {
					if (err) console.log("User field error: " + err);
					NodeBB.db.sortedSetAdd('yuuid:sorted', 0, req.body.uuid + ':' + uid, function (err) {
						if (err) console.log("Sorted Set error: " + err);
