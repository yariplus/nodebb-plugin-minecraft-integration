...
API.init = function () {
	...
	// API Functions
	function getServerStatus(req, data, next) {
		Config.getServerStatus((data && data.sid) ? data.sid : (req && req.params && req.params.sid) ? req.params.sid : 0, function (status) {
			if (data.json) {
				data.json(status);
			}else{
				next(null, status);
	...
	// HTTP API
	NodeBB.router.get('/api/minecraft-integration/server/:sid',       getServerStatus);
	NodeBB.router.get('/api/minecraft-integration/server/:sid/pings', function (req, res, next) { Config.getRecentPings(req.params.sid, 10, function (pings) { res.json(pings); }); });
