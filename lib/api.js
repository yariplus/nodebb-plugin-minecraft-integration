...
API.init = function () {
	...
	function addToWriteAPI(method, name, path, respondWith) {
		...
		NodeBB.SocketPlugins.MinecraftIntegration[name] = function (socket, data, next) {
			...
			Backend.getSidUsingAPIKey(data.key, function (err, sid) {
				...
				if (err) return next("Invalid API key.", {});
				if (!sid) return next("Invalid API key.", {});
	...
	addToAPI(Backend.getPlayer,               'getUserPlayers',          'players/uid/:uid');

	addToAPI(Backend.getUsers,                'getUsers',                'users');
	addToAPI(Backend.getUser,                 'getUser',                 'users/uuid/:id');
	addToAPI(Backend.getUser,                 'getUser',                 'users/name/:name');
