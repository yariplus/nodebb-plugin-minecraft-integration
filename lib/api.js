...
API.init = function () {
	...
	NodeBB.SocketPlugins.MinecraftIntegration.PlayerChat = function (socket, data, callback) {
	...
	NodeBB.SocketPlugins.MinecraftIntegration.getChat = function (socket, data, callback) {
		var sid = data.sid;
...
API.register = function (req, res) {
	if ('https' == req.protocol) {
		...
		if (Config.settings.get('api-key').slice(0, 8) === 'INSECURE') {
