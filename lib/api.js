...
API.init = function () {
	...
	// function PlayerChat(data, next) {
		// if (!(data && data.player && data.message)) return next();
		...
		// NodeBB.db.increment('mi:server:' + sid + ':cid', function (err, cid) {
			// NodeBB.db.sortedSetAdd('mi:server:' + sid + ':cid:time', Date.now(), cid, function (err) {
				// NodeBB.db.setObject('mi:server:' + sid + ':chat:' + cid, {playername: player, message: message}, function (err) {
					// Controller.sendPlayerChatToUsers({sid: sid, chat: {playername: player, message: message}});
					// next();
				// });
			// });
		// });
	...
	addToWriteAPI(register, 'register', 'register/:id/:name/:pass');
