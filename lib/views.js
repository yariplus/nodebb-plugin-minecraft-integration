...
Views.modifyTemplates = function (callback)
{
	...
	async.parallel({
		...
		{
			// Vanilla/Lavender
			tpl = tpl.replace('<i component="user/status" class="fa fa-circle status {status}" title="[[global:{status}]]"></i>', '<span class="h1">{prefix}</span><br><i component="user/status" class="fa fa-circle status {status}" title="[[global:{status}]]"></i>');
