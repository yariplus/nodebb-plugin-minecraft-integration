	...
	function renderMinecraftProfile(req, res, next) {
		...
		async.waterfall([
			...
			function (userData, next) {
				...
				NodeBB.db.getSortedSetRangeByScore('playerkey:uid', 0, 1, req.uid, req.uid, function (err, key) {
					if (err || !key || !key.length) {
						payload.playerKey = Utils.getKey();
						NodeBB.db.sortedSetAdd('playerkey:uid', req.uid, payload.playerKey);
					}else{
						payload.playerKey = key[0];
					}
					return next();
				});
			}
		], function (err, minecraftProfile) {
			if (err) res.end(err); // TODO
...
Views.modifyTemplates = function (callback)
{
	...
	async.parallel({
		...
		{
			// Persona
			tpl = tpl.replace('<h1 class="fullname"><!-- IF fullname -->{fullname}<!-- ELSE -->{username}<!-- ENDIF fullname --></h1>', '<h1 class="prefix" <!-- IF !prefix -->style="display:none;"<!-- ENDIF !prefix -->>{prefix} {username}</h1><h1 class="fullname"><!-- IF fullname -->{fullname}<!-- ELSE -->{username}<!-- ENDIF fullname --></h1>');

			// Vanilla/Lavender
			tpl = tpl.replace('<i component="user/status" class="fa fa-circle status {status}" title="[[global:{status}]]"></i>', '<span class="h4" <!-- IF !prefix -->style="display:none;"<!-- ELSE -->style="display:block;margin-bottom:0;"<!-- ENDIF !prefix -->>{prefix}</span><i component="user/status" class="fa fa-circle status {status}" title="[[global:{status}]]"></i>');
		}
