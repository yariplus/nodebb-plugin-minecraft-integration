...
module.exports = function (API) {
	...
	API.eventPlayerJoin = function (data, callback) {
		...
		var usePrimaryPrefixOnly = Config.settings.get('usePrimaryPrefixOnly');
		var newPrefix = '';
		var primaryPrefix = '';
		...
		NodeBB.db.setObjectField('yuuid:' + id, 'prefix', usePrimaryPrefixOnly ? primaryPrefix : newPrefix);
		...
		async.parallel({
			...
			user: function (next) {
        // TEMP
				API.getUser({id: id}, function (err, user) {
					next(null, user || null);
				});
			}
		}, callback);
