					...
					async.each(_status.players, function (player, next) {
						async.parallel({
							playtime: function (next) {
								NodeBB.db.getObjectField('yuuid:' + Utils.trimUUID(player.id), 'lastonline', function (err, data) {
									if (parseInt(data) !== updateTime) {
										NodeBB.db.setObjectField('yuuid:' + Utils.trimUUID(player.id), 'lastonline', updateTime);
										NodeBB.db.incrObjectField('yuuid:' + Utils.trimUUID(player.id), 'playtime', next);
									}else{
										NodeBB.db.getObjectField('yuuid:' + Utils.trimUUID(player.id), 'playtime', next);
									}
								});
							},
							playername: async.apply(NodeBB.db.setObjectField, 'yuuid:' + Utils.trimUUID(player.id), 'playername', player.name)
						}, function (err, results) {
							if (err) {
								console.log('[Minecraft Integration] Error setting player object ' + player.id + ': ' + err);
							}else{
								NodeBB.db.sortedSetAdd('yuuid:playtime', results.playtime || "0", Utils.trimUUID(player.id), function (err) {
								});
							}
						});
					});
